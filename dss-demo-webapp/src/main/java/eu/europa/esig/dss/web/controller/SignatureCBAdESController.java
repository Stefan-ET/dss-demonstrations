...
@Controller
@SessionAttributes(value = { "signatureCBAdESForm", "signedCBAdESDocument" })
@RequestMapping(value = "/sign-with-cb-ades")
public class SignatureCBAdESController extends AbstractSignatureController {

    private static final Logger LOG = LoggerFactory.getLogger(SignatureCBAdESController.class);

    private static final String SIGNATURE_CBAdES = "signature-cb-ades";

    private static final String[] ALLOWED_FIELDS = { "documentsToSign", "coseStructureType", "tagged", "signaturePackaging",
            "signatureLevel", "sigDMechanism", "externallySuppliedData", "digestAlgorithm", "signWithExpiredCertificate",
            "addContentTimestamp" };

    @InitBinder
    public void initBinder(WebDataBinder webDataBinder) {
        webDataBinder.registerCustomEditor(COSESignatureType.class, new EnumPropertyEditor(COSESignatureType.class));
    ...
    public String showSignatureParameters(Model model, HttpServletRequest request) {
        SignatureCBAdESForm signatureCBAdESForm = new SignatureCBAdESForm();

        // Pre-configure for CB-AdES
        signatureCBAdESForm.setSignatureForm(SignatureForm.CBAdES);
        signatureCBAdESForm.setSignatureLevel(SignatureLevel.CB_AdES_BASELINE_B);
        signatureCBAdESForm.setDigestAlgorithm(DigestAlgorithm.forName(defaultDigestAlgo, DigestAlgorithm.SHA256));
        signatureCBAdESForm.setCoseStructureType(COSEStructureType.COSE_SIGN1);

        model.addAttribute("signatureCBAdESForm", signatureCBAdESForm);
        return SIGNATURE_CBAdES;
    }

    @RequestMapping(method = RequestMethod.POST)
    public String sendSignatureParameters(Model model, HttpServletRequest response,
                                          @ModelAttribute("signatureCBAdESForm") @Valid SignatureCBAdESForm signatureCBAdESForm,
        ...
        model.addAttribute("signatureCBAdESForm", signatureCBAdESForm);
        model.addAttribute("digestAlgorithm", signatureCBAdESForm.getDigestAlgorithm());
        model.addAttribute("rootUrl", "sign-with-cb-ades");
    ...
    public GetDataToSignResponse getDataToSign(Model model, @RequestBody @Valid DataToSignParams params,
                                               @ModelAttribute("signatureCBAdESForm") @Valid SignatureCBAdESForm signatureCBAdESForm,
                                               BindingResult result) {
        signatureCBAdESForm.setCertificate(params.getSigningCertificate());
        signatureCBAdESForm.setCertificateChain(params.getCertificateChain());

        CertificateToken signingCertificate = DSSUtils.loadCertificate(params.getSigningCertificate());
        signatureCBAdESForm.setEncryptionAlgorithm(EncryptionAlgorithm.forName(signingCertificate.getPublicKey().getAlgorithm()));
        signatureCBAdESForm.setSigningDate(new Date());

        if (signatureCBAdESForm.isAddContentTimestamp()) {
            signatureCBAdESForm.setContentTimestamp(WebAppUtils.fromTimestampToken(signingService.getContentTimestamp(signatureCBAdESForm)));
        }

        model.addAttribute("signatureCBAdESForm", signatureCBAdESForm);

        ToBeSigned dataToSign = signingService.getDataToSign(signatureCBAdESForm);
        if (dataToSign == null) {
            return null;
        }
    ...
    public SignDocumentResponse signDocument(Model model, @RequestBody @Valid SignResponse signatureValue,
        ...
        signatureCBAdESForm.setSignatureValue(signatureValue.getSignatureValue());

        DSSDocument document = signingService.signDocument(signatureCBAdESForm);
        InMemoryDocument signedCBAdESDocument = new InMemoryDocument(DSSUtils.toByteArray(document), document.getName(), document.getMimeType());
        model.addAttribute("signedCBAdESDocument", signedCBAdESDocument);
    ...
    public String downloadSignedFile(@ModelAttribute("signedCBAdESDocument") InMemoryDocument signedDocument, HttpServletResponse response) {
        try {
    ...
    @ModelAttribute("coseStructureType")
    public COSEStructureType[] getCoseStructureTypes() {
        return COSEStructureType.values();
    }
    ...
    public SigDMechanism[] getSigdMechanisms() {
        return new SigDMechanism[] { SigDMechanism.OBJECT_ID_BY_URI, SigDMechanism.OBJECT_ID_BY_URI_HASH, SigDMechanism.NO_SIG_D };
    }
